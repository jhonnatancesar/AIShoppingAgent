...
`20260808_0006`. Seu contrato funcional e limites estão em `docs/architecture/users.md`.
...
`mission_status` contém somente `draft`, `active`, `paused`, `completed`, `cancelled` e `expired`. As transições válidas continuam sendo as de `docs/architecture/mission-system.md`; o enum isoladamente não as garante.
...
contrato persistente e seus limites estão em `docs/architecture/missions.md`; comandos e
...
`evaluate_price_alerts` (`docs/architecture/price-alerts.md`) para aquela observação;
...
`docs/architecture/mission-transitions.md`.
...
está em `docs/architecture/mission-schedules.md`.
...
conservadora de identidade e os limites funcionais estão em `docs/architecture/products.md`.
...
| --- | --- | --- |
| `id` | `uuid` | Chave primária. |
| `event_type` | `varchar(120)` | Obrigatório; nome versionado de `docs/architecture/event-catalog.md`. |
...
`docs/architecture/event-consumption.md`.
...
- Migrações, metadata ORM, sessões e conexão foram configuradas na TASK-011.
  ...
  estão documentadas em `docs/architecture/price-history.md`.
