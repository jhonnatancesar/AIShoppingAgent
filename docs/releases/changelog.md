...
- **Reescrito para Windows**: `scripts/funnel_healthcheck.ps1` (testa
  ...
  `docs/installation/linux-legacy-setup.md`.
...
- Adicionados `docs/internal/handoff-2026-08-15.md` e
  `docs/database/alembic-check-issue.md` com estado, limites operacionais, reprodução
  segura e critérios de aceite do drift.
- Sincronizadas as memórias permanentes (`CLAUDE.md` e
  `docs/internal/project-context.md`) e o estado atual em `AGENTS.md`.
...
- Validado com pipeline oficial completo (815 testes, 90,69% cobertura,
  ...
  itens do **planejamento original** de `docs/internal/v1.0.2-scope.md` estão
...
- **Escopo da `v1.0.2` ampliado logo em seguida (`DEC-060`):** ao aprovar
  ...
  `5 Todas`). Registrados em `docs/internal/v1.0.2-scope.md` como itens 6 e 7. **A
...
- `docs/architecture/users.md` atualizado; testes reescritos para o vocabulário
...
- `docs/installation/linux-legacy-setup.md` atualizado (seções "Serviços" e "Inicialização
...
- **TASK-065** (`docs/tasks/TASK-065.md`, item 1) concluída: auditoria
  ...
  `docs/installation/linux-legacy-setup.md` atualizados para não anunciar essas duas
...
- **`v1.0.1` implantada num servidor de produção real**: build das imagens,
  ...
  em `docs/installation/linux-legacy-setup.md` (seção 9).
- **Achado real de implantação, corrigido**: os containers da aplicação
  ...
  operacional futura em `docs/installation/linux-legacy-setup.md`.
- **`docs/internal/v1.0.2-scope.md` criado**, separado de `docs/internal/v1.2-scope.md` — a `v1.0.2`
  ...
  - **`docs/internal/v1.0.2-scope.md`** ganhou três itens além dos dois originais do
  ...
  - **`docs/internal/v1.2-scope.md`** (evolução funcional) ganhou três itens: reduzir
...
- Auditoria identificou que a tag `v1.0.0` (`85b56c6`) permaneceu apontando
  ...
  concluídas depois. O `docs/releases/checklist.md` marcado como "65/65"
...
- Documentos sincronizados: `docs/releases/checklist.md` (65/65 associado à
  `v1.0.1`, `v1.0.0` marcada como histórica), `docs/tasks/TASK-054.md`
  (nota posterior sobre a supersessão), `docs/internal/decision-log.md` (decisão de
...
- **TASK-064 concluída**, aprovada explicitamente pelo usuário. Registro do
  ...
  intocada.
- `docs/releases/checklist.md` atualizado: critério 6 do MVP volta a
...
- `docs/releases/checklist.md` fechado como retrato real do repositório:
  63/63 tarefas planejadas concluídas, 8/8 critérios objetivos do MVP
  (`docs/internal/mvp.md`) atendidos.
- Tag Git anotado `v1.0.0` criado sobre o commit revisado e publicado em
  ...
  (permite `docs/operations/linux-runbook.md` fazer checkout por tag no futuro) — sem
...
- `AGENTS.md`, `docs/internal/roadmap.md`, `docs/internal/project-context.md` e
...
- Criado `docs/operations/linux-runbook.md` para preparar e operar um único Ubuntu Server,
...
- Criado inventário de dados, finalidades, terceiros, retenções e limitações em
  `docs/architecture/privacy.md`, sem alegar anonimização irreversível ou certificação LGPD.
...
- `backend/migrations/versions/20260808_0003_create_events.py` (novo): cria
  `events` conforme `docs/database/schema.md` — `event_type`/`aggregate_type` em
...
- Comando `/upgrade` registrado no menu do bot
  ...
  (`docs/internal/out-of-scope.md`: "Plano PLUS", "Usuário pago" continuam na V2).
...
- Ampliar ainda mais a variedade de linguagem testada pelo
  `IntentInterpreter` registrada em `docs/internal/backlog.md` para a V2.
...
- Documentado em `docs/architecture/telegram-adapter.md`, com referência cruzada em
  `docs/architecture/telegram.md`, e registrada a decisão de escopo em
  `docs/internal/decision-log.md` (DEC-009).
...
- Confirmado por auditoria que o vocabulário de `IntentKind` e
  ...
  (`docs/architecture/mission-system.md`), os campos de `MissionCriteria`
  (`docs/architecture/mission-criteria.md`) e as quatro fontes selecionáveis
  (`docs/architecture/telegram.md`), sem nenhum campo novo de domínio.
...
- Aprovados lint e formatação (`ruff check`, `ruff format --check`) e os
  ...
  3.14.6 já validado conforme `docs/development/dependencies.md`.
...
- Registrado em `docs/internal/security-incident-log.md` o evento `INC-2026-08-02-001`, sua resposta, evidências sanitizadas e prevenção.
...
- Criado `docs/development/dependencies.md` como inventário de ferramentas e dependências da aplicação.
...
- Criados `docs/internal/backlog.md`, `docs/internal/mvp.md` e `docs/internal/out-of-scope.md` para controlar ideias futuras, escopo da V1 e exclusões explícitas.
- Adicionadas referências cruzadas a esses documentos no contexto do projeto e no roadmap.
- Criado `docs/internal/decision-log.md` e adicionada política permanente de classificação prévia de novas funcionalidades no `AGENTS.md`.
