...
- Proprietário promovido a `DEV` pelo procedimento manual de
  `docs/PRODUCTION_SETUP.md` (seção 9).
- **Achado real corrigido**: containers da aplicação rodam como usuário
  ...
  não documentado como nota permanente em `docs/PRODUCTION_SETUP.md` —
  pendência leve para uma próxima sessão.
- Endereço, credenciais e detalhes específicos do servidor **não estão
  neste repositório** (por design — `docs/SECRETS.md`) e não estão neste
  handoff.
...
- **`v1.0.2`** — release *patch* dentro da V1 (semver `x.y.z`), corretiva,
  registrada em **`docs/V1_0_2.md`**.
- **V1.2** — fase funcional maior, entre a V1 (MVP completo) e a V2,
  registrada em **`docs/V1_2.md`**.

Ordem: `v1.0.1` (atual, em produção) → `v1.0.2` (`docs/V1_0_2.md`) → V1.2
(`docs/V1_2.md`) → V2. Nenhum item de nenhum dos dois virou TASK; nenhum
código foi alterado por nenhum registro.

### `docs/V1_0_2.md` — 5 itens
...
### `docs/V1_2.md` — 12 itens (itens 1–8 já existiam de sessões anteriores)
...
11. Comparação de menor preço histórico — externo (pesquisa fora do que o
    ...
    é o item 5 de `docs/V1_0_2.md`. **Regra sem exceção**: a IA nunca
...
## Decisões desta sessão (`docs/DECISION_LOG.md`)
...
docs/HANDOFF_V1_0_2.md para reconstruir o estado exatamente de onde
...
v1.0.2 (docs/V1_0_2.md, 5 itens) e V1.2 (docs/V1_2.md, 12 itens) são
