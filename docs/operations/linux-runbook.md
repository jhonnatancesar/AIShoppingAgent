...
em [SECRETS.md](../installation/secrets.md).
...
**não migra agendas já persistidas** (`docs/architecture/mission-schedules.md`): sem o
...
6. registre a data e os valores finais em `docs/internal/project-context.md`.

Backoff persistente por fonte após bloqueio externo (401/403/429) continua
pendente de modelagem e aprovação (`DEC-046`, `docs/architecture/mission-schedules.md`) —
...
- Rotação de credenciais: [SECRETS.md](../installation/secrets.md).
- Incidente de vazamento: interrompa a operação afetada, revogue/rotacione no
  ...
  [SECURITY_INCIDENT_LOG.md](../internal/security-incident-log.md).
- Limpeza manual de tokens/sessões antigos e desidentificação controlada:
  [PRIVACY.md](../architecture/privacy.md). Desidentificação é operação sensível, confirmada e
