...
|---|---|---|---|
| GG Oferta | **`v1.3.23`** | `163aa09` (`main`) | Substitui `v1.3.17`–`v1.3.22`. Terceira/quarta rodadas de `--dry-run` real em PROD (sob `v1.3.22`, log já legível) acharam duas falhas DIFERENTES no mesmo lote em tentativas seguidas: `502` do próprio César Core numa, e o fallback `openrouter/free` devolvendo texto solto (`"User Safety: safe"`, não o JSON pedido) na outra. Causa real (achado do usuário): o modo em lote mandava um array JSON cru como mensagem inteira do usuário, sem frase em linguagem natural ancorando a tarefa -- formato atípico pra um modelo mais fraco/gratuito; o modo de item único sempre manda texto natural e nunca teve esse problema. Corrigido -- mensagem do lote agora ancora a instrução em linguagem natural antes do JSON. **Validação real (nova chamada de IA em PROD) ainda pendente** -- ver seção "TASK-123 -- ação obrigatória no deploy" logo abaixo desta tabela |
| GG Oferta (histórico) | `v1.3.15`–`v1.3.22` | `e8ac138`/`d071adc`/`ba3ef16`/`2155a21`/`204da8a`/`dcfda07`/`76e37a9`/`015979c` | Identidade global de produto (SKU vs part number + árbitro de IA), parcelamento real, `offer_supersession`, `coupon_evidence_isolation`, `search_history` + área DEV, cobertura de testes ≥90%, correção do início do Windows Ops Agent, fechamento documental do episódio `INC-2026-09-17-001` (`v1.3.15`); correções de sequência de tag sem mudança de código (`v1.3.16`, `v1.3.18`); TASK-122 (busca sem oferta relevante não é mais bloqueio) + TASK-123 original -- script de backfill sem lote (`v1.3.17`); TASK-123 com lote de 4/teto de 20 (`v1.3.19`); TASK-123 com lote de 10/teto de 100, ajustado ao backlog real de 371 (`v1.3.20`); TASK-123 corrige relatório contraditório do dry-run + melhora log de falha de lote (`v1.3.21`); TASK-123 liga logging estruturado no script (`v1.3.22`). Substituem `v1.3.8` (handoff parado desde então, v1.3.9-v1.3.14 foram hotfixes pontuais não documentados aqui) |
...
repositórios que existem em PROD (`git checkout v1.3.23` — confirme que é
...
`v1.3.23`), que já referencia `ghcr.io/jhonnatancesar/cesar-core:1.2.1`
