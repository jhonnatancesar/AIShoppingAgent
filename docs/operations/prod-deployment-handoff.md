...
|---|---|---|---|
| GG Oferta | **`v1.3.21`** | `76e37a9` (`main`) | Substitui `v1.3.17`–`v1.3.20`. Achado real de um `--dry-run` contra o banco de PROD (371 Products, sob `v1.3.20`): relatório contraditório (resumo dizia "13 resolvidos", detalhe mostrava 0) causado por `session.rollback()` de um lote descartando da sessão a resolução de um lote anterior em `--dry-run` -- corrigido com `outcome_sink` (populado no momento da resolução, nunca depende do estado da sessão sobreviver a rollback). Também achado: 9 de ~10 lotes falharam a extração sem causa visível no log -- log agora separa falha de chamada (`stage=generate`) de resposta fora do contrato (`stage=parse`, com preview do conteúdo). Nada foi aplicado em PROD -- ver seção "TASK-123 -- ação obrigatória no deploy" logo abaixo desta tabela |
| GG Oferta (histórico) | `v1.3.15`–`v1.3.20` | `e8ac138`/`d071adc`/`ba3ef16`/`2155a21`/`204da8a`/`dcfda07` | Identidade global de produto (SKU vs part number + árbitro de IA), parcelamento real, `offer_supersession`, `coupon_evidence_isolation`, `search_history` + área DEV, cobertura de testes ≥90%, correção do início do Windows Ops Agent, fechamento documental do episódio `INC-2026-09-17-001` (`v1.3.15`); correções de sequência de tag sem mudança de código (`v1.3.16`, `v1.3.18`); TASK-122 (busca sem oferta relevante não é mais bloqueio) + TASK-123 original -- script de backfill sem lote (`v1.3.17`); TASK-123 com lote de 4/teto de 20 (`v1.3.19`); TASK-123 com lote de 10/teto de 100, ajustado ao backlog real de 371 (`v1.3.20`). Substituem `v1.3.8` (handoff parado desde então, v1.3.9-v1.3.14 foram hotfixes pontuais não documentados aqui) |
...
repositórios que existem em PROD (`git checkout v1.3.21` — confirme que é
...
`v1.3.21`), que já referencia `ghcr.io/jhonnatancesar/cesar-core:1.2.1`
