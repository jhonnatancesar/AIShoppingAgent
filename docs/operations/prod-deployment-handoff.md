...
|---|---|---|---|
| GG Oferta | **`v1.3.22`** | `015979c` (`main`) | Substitui `v1.3.17`–`v1.3.21`. Segunda rodada de `--dry-run` real em PROD (sob `v1.3.21`) confirmou o `outcome_sink` corrigido, mas achou que os campos de diagnóstico (`stage`/`error`/`raw_content_preview`) nunca apareciam na saída -- causa: o script nunca chamava `configure_logging` (JSON estruturado em stdout), sem handler configurado o `logging` padrão do Python descarta `extra={}`. Corrigido -- confirmado manualmente que os campos aparecem agora. Causa raiz da falha de extração em si (9 de ~10 lotes falhando) continua desconhecida -- só agora vai dar pra ver no log de uma próxima rodada. Nada foi aplicado em PROD -- ver seção "TASK-123 -- ação obrigatória no deploy" logo abaixo desta tabela |
| GG Oferta (histórico) | `v1.3.15`–`v1.3.21` | `e8ac138`/`d071adc`/`ba3ef16`/`2155a21`/`204da8a`/`dcfda07`/`76e37a9` | Identidade global de produto (SKU vs part number + árbitro de IA), parcelamento real, `offer_supersession`, `coupon_evidence_isolation`, `search_history` + área DEV, cobertura de testes ≥90%, correção do início do Windows Ops Agent, fechamento documental do episódio `INC-2026-09-17-001` (`v1.3.15`); correções de sequência de tag sem mudança de código (`v1.3.16`, `v1.3.18`); TASK-122 (busca sem oferta relevante não é mais bloqueio) + TASK-123 original -- script de backfill sem lote (`v1.3.17`); TASK-123 com lote de 4/teto de 20 (`v1.3.19`); TASK-123 com lote de 10/teto de 100, ajustado ao backlog real de 371 (`v1.3.20`); TASK-123 corrige relatório contraditório do dry-run + melhora log de falha de lote (`v1.3.21`). Substituem `v1.3.8` (handoff parado desde então, v1.3.9-v1.3.14 foram hotfixes pontuais não documentados aqui) |
...
repositórios que existem em PROD (`git checkout v1.3.22` — confirme que é
...
`v1.3.22`), que já referencia `ghcr.io/jhonnatancesar/cesar-core:1.2.1`
...
1. **Rodar o backfill contra o backlog real** (produtos já coletados
   ...
   ```
   **Leia a saída com atenção real, não só o resumo** -- achado real em
   PROD (`v1.3.20`): um `--dry-run` chegou a mostrar "Resolvidos nesta
   rodada: 13 de 100" no resumo enquanto o detalhe por produto mostrava
   0 "RESOLVIDO" e 100 "continua sem identidade" -- contradição real
   (bug corrigido em `v1.3.21`, `outcome_sink`), não suposição. Se o
   resumo e o detalhe não baterem, ou se aparecerem várias mensagens de
   log `product_identity_ai_batch_extraction_failed` (log agora traz
   `stage="generate"`/`stage="parse"` + preview do conteúdo pra
   diagnosticar a causa real), **pare e reporte antes de `--apply`** --
   não é seguro presumir que passou. Confirme que os candidatos
   listados fazem sentido (nomes de produto reais, sem lixo) e que a
   extração da IA (coluna `RESOLVIDO -> category=...`) parece plausível
