...
- `command` só é aceito quando `kind` é `mission_command` e reaproveita
  diretamente `MissionCommand` de `docs/architecture/mission-system.md`
...
- `IntentParameters` reaproveita conceitos já existentes: `search_query` e o
  ...
  (`docs/architecture/mission-criteria.md`), `sources` aceita somente as quatro fontes
  selecionáveis da V1 (`pichau`, `terabyte`, `amazon`, `kabum`, conforme
  `docs/architecture/telegram.md`) e `mission_reference` é um texto livre para identificar
...
- `create_mission` e `query_mission` cobrem exatamente a exigência de
  `docs/internal/mvp.md`: "Um usuário autorizado consegue criar e consultar uma missão
  pelo canal Telegram".
