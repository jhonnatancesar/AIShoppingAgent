...
`docs/architecture/event-catalog.md` e no contrato executável `app.events`. Persistência
...
`docs/architecture/event-consumption.md`. Detecção de fatos além dos alertas de preço
...
`docs/architecture/price-alerts.md`, `docs/architecture/event-consumption.md` e as TASKs 043–044.

A TASK-062 publica, pelo serviço existente, candidatos de preço e os fatos
`collection.completed.v1`/`collection.failed.v1` produzidos pelo orquestrador.
Ela não altera o catálogo nem o consumidor atual e nunca chama Telegram
diretamente.
