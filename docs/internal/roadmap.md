...
| Fila justa e controle de carga | TASK-108 | **Em pausa por decisão do usuário (2026-08-22)** — planejamento corrigido e aprovado (cooldown 1–3 min por usuário, `DEC-095`); implementação parcial iniciada (`UserCollectionQueueState`, seleção justa em `claim_due_collections`) mas **não commitada**, com 2 testes de integração falhando (regressão real ainda não diagnosticada) — retomar só depois de investigar antes de continuar |
| Migrar collection_worker para Windows nativo com Edge | TASK-109 | **Concluída no DEV, não implantada** — worker nativo Windows (Task Scheduler + Windows Ops Agent), Edge via CDP loopback direto (sem bridge) com lifecycle sob demanda (lease/idle-timeout), `BrowserSession`/Chromium removidos de todo caminho real de coleta (só permanece como infraestrutura de teste hermética), `collection_worker` removido do `compose.yaml`, `ops_controller` integrado ao Windows Ops Agent (`WindowsOpsAgentAdapter`) (`DEC-096`) |
