...
| Avaliação inteligente de preço, pesquisa de mercado e qualidade dos alertas — **absorve o item 17 da V1.2** (menor preço histórico externo) | TASK-113 | **Registro e desenho inicial fechado commitados localmente (`fecd804`, 2026-08-27).** Pré-flight (§32) executado, `docs/tasks/TASK-113.md` §33 é a fonte de verdade — chave por `product_id` (Product Identity Engine, TASK-097), checkpoint `MissionProductAlertState` por `(mission_id, product_id)`, `MarketPriceAssessment` com single-flight crash-safe (lease), TTL/re-alert/material improvement determinísticos, Firecrawl + AI Provider Manager, gatilho determinístico. **Correção posterior (§38, working tree, ainda sem commit)**: pesquisa de histórico externo passou a ser parte do mesmo `MarketPriceAssessment` (busca Firecrawl dedicada), absorvendo formalmente o item 17 da V1.2 — não é mais item sem TASK. **Implementação (código/migration) ainda não iniciada** em nenhuma das duas rodadas |
