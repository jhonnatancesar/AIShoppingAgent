...
| Migrar collection_worker para Windows nativo com Edge | TASK-109 | Formalizada (preflight + plano); Task Scheduler, Edge via CDP loopback direto (sem bridge), `ops_controller` já tem abstração pronta (`DockerOpsAdapter` isolado), validação por loja antes de remover Chromium (`DEC-096`) |
