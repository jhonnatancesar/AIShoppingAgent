...
| Pesquisa de cupons | TASK-106 | Reorientada como subsistema independente — Coupon Collector, modelo e avaliação de aplicabilidade separados do `collection_worker`, nunca acoplados à coleta de oferta (`DEC-093`). Auditoria real (Amazon/Kabum/Magalu/ML) concluída; arquitetura proposta, aguardando aprovação para código (`DEC-088`) |
| Histórico e gráficos por produto/variante | TASK-098 | Formalizada e reservada como último item da V1.2 (`DEC-081`/`DEC-082`) |
| Cotas e capacidade por usuário | TASK-107 | Formalizada (planejamento); `max_active_missions=5`, `max_store_slots=18`, `max_daily_searches=30`, nunca pausa/cancela automaticamente, UX de uso/limite obrigatória, override por ADMIN (`DEC-094`) |
| Fila justa e controle de carga | TASK-108 | Formalizada (planejamento); fila por usuário com cooldown individual (10–15 min), `coupon_worker` fora da fila, sem alterar limites por provider (`DEC-095`) |
