...
| Corrigir asserção desatualizada em `test_product_identity.py` | TASK-111 | Formalizada, não iniciada — teste espera só 4 lojas seed (falta Magalu/Mercado Livre), achado durante a validação da TASK-108, escopo independente dela |
| Vincular missões que monitoram o mesmo item, sem duplicar coleta | TASK-112 | Desenho arquitetural revisado (2026-08-24), aguardando aprovação — base passa a ser um Product Identity Engine genérico (registry de categorias/atributos, expandindo `app/products/identity.py`/TASK-097, não extractors pontuais), IA nunca decide equivalência, `monitoring_key` determinística com `ANY` explícito por atributo; `MonitoringItem`/`MonitoringItemSchedule`/`MonitoringItemSource` novos, fan-out reaproveitando `MissionOfferRelevance` já por missão (TASK-063); fairness da TASK-108 revisada para `fairness_owner` único por claim compartilhada, não crédito a todos os vinculados. Nenhum código escrito |
