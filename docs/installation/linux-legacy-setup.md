...
Este guia **não substitui** `docs/operations/linux-runbook.md` (runbook de operação
contínua — rotina, rollback, diagnóstico, rotação de credenciais) nem
`docs/installation/secrets.md` (gestão detalhada de secrets) nem `docs/database/migrations.md`
...
- **Os dados persistentes vêm do PostgreSQL real da produção**, criado vazio
  nesta instalação, ou de um backup de produção já validado (`docs/operations/linux-runbook.md`)
...
O que o projeto realmente usa (verificado em `docs/development/dependencies.md`,
`docs/operations/linux-runbook.md` e `compose.yaml`):
...
Dimensionamento observado pelo próprio projeto (`docs/operations/linux-runbook.md`, DEC-046):
...
Permitir que o usuário operacional use o Docker sem `sudo` — `docs/operations/linux-runbook.md`
...
Não use branch mutável em produção. `docs/operations/linux-runbook.md` reforça: nunca
...
| `AISHOPPING_COLLECTION_SCHEDULE_INTERVAL_MINUTES` | Intervalo entre coletas da mesma missão | Opcional | `30` (temporário, DEC-046, servidor de 8 GB) | Não altere para `15` sem seguir o procedimento de "Upgrade de 8 GB para 16 GB" em `docs/operations/linux-runbook.md` — mudar só a env var não migra agendas já persistidas |
...
`docs/installation/secrets.md` é a fonte de verdade completa (inventário, rotação,
...
Menor privilégio por serviço (verificado em `compose.yaml`, tabela completa em `docs/installation/secrets.md`):
...
mecanismo usado em desenvolvimento (`docs/installation/secrets.md`), rodando aqui contra
...
compartilhe a cota entre os dois (`docs/architecture/ai-provider-manager.md`).
...
- **permanecem só no servidor** onde foram criados. Não copie `.secrets/`
  ...
  rotação de credencial (`docs/installation/secrets.md`) ou gere novos valores no destino.
...
banco com dados (`docs/database/migrations.md`, `docs/operations/linux-runbook.md`).
...
`DEV`** (`docs/architecture/users.md`, `docs/tasks/TASK-047.md`). O projeto **não possui,
...
perfil `USER` (`docs/architecture/ai-provider-manager.md`).
...
> guia** nem do escopo já aprovado da V1 (`docs/operations/linux-runbook.md` é explícito:
...
- [ ] Fallback Groq funcionando quando aplicável (não force isso artificialmente; se aparecer `ai_outcome=quota_exceeded`/`unavailable` seguido de `succeeded` via Groq nos logs, o fallback está funcionando — `docs/architecture/ai-provider-manager.md`);
...
aplicação. Traces do Jaeger são voláteis por design (`docs/operations/linux-runbook.md`) e
...
Baseado no procedimento já suportado e documentado em `docs/operations/linux-runbook.md` —
...
produção (`docs/operations/linux-runbook.md`):
...
como um rollback avaliado manualmente (`docs/operations/linux-runbook.md`, seção
...
Os logs são JSON estruturado em `stdout` (`docs/operations/logging.md`); não copie logs
...
- [ ] Nenhuma chave gravada desnecessariamente em comandos (nunca `echo`/`cat` de secret, nunca secret como argumento de linha de comando — `docs/installation/secrets.md`);
...
- [ ] Somente as portas realmente necessárias abertas — hoje isso é só `22/tcp` (SSH) e, quando a infraestrutura de domínio/TLS existir, `443`/`80`; `8000`, `5432`, `9090`, `16686`, `13133` continuam em loopback (`docs/operations/linux-runbook.md`, tabela de portas);
...
- [ ] Logs sem secrets (a aplicação já sanitiza por design — `docs/operations/logging.md` — mas revise antes de compartilhar qualquer trecho).
...
projeto não define essa infraestrutura hoje (`docs/operations/linux-runbook.md`: "não há
...
diagnóstico avançado, rollback): `docs/operations/linux-runbook.md`. Gestão detalhada de
secrets: `docs/installation/secrets.md`. Migrations: `docs/database/migrations.md`.
