...
Por que isto existe (achado real, 2026-09-08): o Windows não tem
equivalente a SIGKILL capturável -- quando o processo do worker é morto
abruptamente (`Stop-ScheduledTask`/`Stop-Process -Force`, crash, falta de
energia), nenhum código Python roda para fechar o Edge que ele lançou
(`EdgeCdpSupervisor._terminate_launcher` nunca é alcançado). Um Job Object
com `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE` resolve isso no nível do kernel,
sem depender de nenhum código rodar: quando o ÚLTIMO handle do job fecha
...
`pywin32`. Requer nested job objects, suportado nativamente desde
...
logger = logging.getLogger("app.collection.job_object")
...
class JobObjectError(RuntimeError):
    ...
    silêncio pelo módulo; o chamador (`EdgeCdpSupervisor`) decide se trata
...
class EdgeLifecycleJob:
    ...
    já estiverem mortos, ex.: depois de `_terminate_launcher` já ter
    encerrado tudo de forma graciosa). O caso real que este objeto existe
    para cobrir nunca chama `close()`: o processo Python morre primeiro, e
    o próprio Windows fecha o handle (e mata a árvore) sem nenhum código
    rodar."""
