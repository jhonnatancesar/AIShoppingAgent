...
- Atualize `docs/internal/project-context.md` e `docs/releases/changelog.md` ao concluir uma tarefa.
...
Após classificar, registre a decisão em `docs/internal/decision-log.md` e atualize automaticamente a documentação correspondente. Proteja sempre o escopo definido em `docs/internal/mvp.md` e evite aumento de complexidade desnecessário. A classificação não substitui a necessidade de uma TASK explicitamente solicitada para implementar qualquer funcionalidade.
...
release (`docs/releases/checklist.md`, 63/63) e publicou o tag Git anotado
...
resolvida quanto ao código (`docs/releases/checklist.md`, 65/65) — mas
...
passou a ser a referência de release. `docs/installation/linux-legacy-setup.md` documenta
...
confundir as versões: **`docs/internal/v1.0.2-scope.md`** (release corretiva `v1.0.2`,
...
**`docs/internal/v1.2-scope.md`** (evolução funcional V1.2, 12 itens) — redução de
...
vigente: `v1.0.1` (atual, em produção) → `v1.0.2` (`docs/internal/v1.0.2-scope.md`,
corretiva) → V1.2 (`docs/internal/v1.2-scope.md`, funcional) → V2. Não iniciar qualquer
TASK de `v1.0.2`/V1.2/V2 sem pedido explícito.

**Atualização 2026-08-10:** por pedido explícito do usuário, a `v1.0.2`
entrou em **planejamento ativo**. Os 5 itens de `docs/internal/v1.0.2-scope.md` foram
...
documentação (`docs/development/dependencies.md`, `docs/installation/linux-legacy-setup.md`) que as
...
`docs/internal/v1.0.2-scope.md` como itens 6 e 7): impedir `/cadastro` para usuário já
...
`docs/database/alembic-check-issue.md`. O handoff completo está em
`docs/internal/handoff-2026-08-15.md`. Nenhuma nova TASK foi iniciada.
...
Telegram agrupada por loja. Foi aprovada e publicada em
`origin/main` no commit `b915106`; nenhum deploy foi feito. A atividade seguinte
é a TASK-095, página rica de oferta (item 5).

**Atualização DEV 2026-08-22 (TASK-095):** o item 5 da V1.2 implementa a
