...
function UserRow({user,reload}:{user:AdminUser;reload:()=>Promise<void>}){const [missions,setMissions]=useState<AdminMission[]|null>(null);return <div className="rounded-xl border p-4"><div className="flex flex-wrap items-center gap-3"><div className="min-w-48 flex-1"><p className="font-medium">{user.display_name}</p><p className="text-xs text-muted-foreground">@{user.username||'removido'} · {user.mission_count} missões</p><p className="text-xs text-muted-foreground">Cota: {quotaLabel(user.max_active_missions_override)} missões · {quotaLabel(user.max_store_slots_override)} lojas · {quotaLabel(user.max_daily_searches_override)} pesquisas/dia</p></div><Status value={user.lifecycle_status}/><Select value={user.role} onValueChange={async role=>{if(confirm(`Alterar papel para ${role}?`)){await adminApi.updateUser(user.id,{role,reason:'Alteração confirmada no painel'});await reload()}}}><SelectTrigger className="w-28"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="USER">USER</SelectItem><SelectItem value="DEV">DEV</SelectItem><SelectItem value="ADMIN">ADMIN</SelectItem></SelectContent></Select><Button size="sm" variant="outline" onClick={()=>editQuotas(user,reload)}>Cotas</Button><Button size="sm" variant="outline" onClick={async()=>setMissions(await adminApi.missions(user.id)||[])}>Missões</Button>{user.lifecycle_status==='active'?<><Button size="sm" variant="outline" onClick={()=>lifecycle(user,'inactive',reload)}>Desativar</Button><Button size="sm" variant="destructive" onClick={()=>lifecycle(user,'blocked',reload)}>Bloquear</Button></>:<Button size="sm" onClick={()=>lifecycle(user,'active',reload)}>Ativar</Button>}<Button size="sm" variant="destructive" onClick={()=>remove(user,reload)}>Remover</Button></div>{missions?<div className="mt-3 space-y-2 border-t pt-3">{missions.length?missions.map(m=><div key={m.id} className="flex items-center justify-between text-sm"><span>{m.title} · {m.status}</span><div className="flex gap-1">{m.status==='active'?<Button size="sm" variant="ghost" onClick={()=>mission(m,'pause',setMissions,user.id)}>Pausar</Button>:m.status==='paused'?<Button size="sm" variant="ghost" onClick={()=>mission(m,'resume',setMissions,user.id)}>Retomar</Button>:null}{m.status==='active'?<Button size="sm" variant="ghost" onClick={()=>collect(m)}>Coletar</Button>:null}{!['cancelled','completed','expired'].includes(m.status)?<Button size="sm" variant="ghost" onClick={()=>mission(m,'cancel',setMissions,user.id)}>Cancelar</Button>:null}</div></div>):<p className="text-sm text-muted-foreground">Nenhuma missão.</p>}</div>:null}</div>}
function quotaLabel(value?:number|null){return value==null?'padrão':String(value)}
// TASK-107: tri-state por campo -- cancelar o prompt não altera aquele
// campo; vazio limpa o override (volta ao padrão do sistema); número
// define o override. Cada campo é perguntado e aplicado só se confirmado.
async function editQuotas(user:AdminUser,reload:()=>Promise<void>){
 const body:Record<string,unknown>={}
 const fields:[string,string,number|null|undefined][]=[
  ['max_active_missions_override','Máximo de missões ativas (vazio = padrão do sistema)',user.max_active_missions_override],
  ['max_store_slots_override','Máximo de lojas monitoradas (vazio = padrão do sistema)',user.max_store_slots_override],
  ['max_daily_searches_override','Máximo de pesquisas por dia (vazio = padrão do sistema)',user.max_daily_searches_override],
 ]
 for(const [field,label,current] of fields){
  const raw=prompt(label,current==null?'':String(current))
  if(raw===null)continue // cancelou -- não mexe neste campo
  if(raw.trim()===''){body[field]=null;continue} // limpa o override
  const parsed=Number(raw)
  if(!Number.isInteger(parsed)||parsed<=0){alert(`Valor inválido para "${label}" -- ignorado.`);continue}
  body[field]=parsed
 }
 if(Object.keys(body).length===0)return
 body.reason='Cota ajustada no painel'
 await adminApi.updateUser(user.id,body);await reload()
}
