  ...
  return (
    <section>
      ...
      <form onSubmit={submit} className="mb-6 grid gap-3 rounded-xl border border-border bg-card p-4 shadow-card lg:grid-cols-[minmax(14rem,1fr)_repeat(4,minmax(8rem,auto))_auto]">
        ...
        <FilterSelect label="Loja" value={filters.store || ''} onChange={(value) => updateFilter('store', value)} options={[['amazon', 'Amazon'], ['kabum', 'KaBuM!'], ['magalu', 'Magalu'], ['mercadolivre', 'Mercado Livre'], ['pichau', 'Pichau'], ['terabyte', 'Terabyte']]} />
...
function OfferCard({ offer, index }: { offer: OfferSummary; index: number }) {
  ...
  const { src: imageSrc, onError: onImageError } = useImageFallbackChain([
    offer.image_url,
    offer.image_fallback_url,
  ])
  return <motion.div initial={prefersReducedMotion ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(index * .025, .2) }}><Card className="flex h-full flex-col overflow-hidden transition-transform hover:-translate-y-0.5"><div className="grid h-44 place-items-center bg-muted/40 p-4">{imageSrc ? <img className="h-full w-full min-h-0 object-contain" src={imageSrc} alt="" onError={onImageError} /> : <ShoppingBag className="size-10 text-muted-foreground/35" />}</div><CardHeader className="flex-1"><div className="flex items-center justify-between gap-2"><Badge variant="secondary">{offer.store.name}</Badge>{offer.rating ? <span className="text-xs text-muted-foreground">★ {Number(offer.rating.average).toLocaleString('pt-BR')}</span> : null}</div><CardTitle className="line-clamp-2 text-base leading-snug">{offer.title}</CardTitle>{observation ? <><p className="text-xl font-semibold tracking-tight">{money(observation.amount, observation.currency)}</p><CardDescription className="flex items-center gap-1.5">Total {money(observation.total_amount, observation.currency)} · <ConditionBadge condition={observation.condition} /></CardDescription></> : <CardDescription>Dados comerciais ainda não disponíveis.</CardDescription>}{offer.seller ? <p className="text-xs text-muted-foreground">Vendido por {offer.seller.name}</p> : null}</CardHeader><CardFooter><Button variant="outline" className="w-full" asChild><Link to={`/app/offers/${offer.id}`}>Ver detalhes <ArrowRight /></Link></Button></CardFooter></Card></motion.div>
}
